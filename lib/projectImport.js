...
const { chatJson, resolveLlm, describeImage: llmDescribeImage } = require('./llm.js');
...
async function describeImage(imageBase64) {
  ...
  const model = config.ollama?.visionModel || llm.model || 'llava';
  return llmDescribeImage(llm, model, raw, 'Describe this image in detail so the description can be used as context for questions later. Include any text, data, or meaning visible.');
}
...
/**
 * Extract text from a Word 97-2003 .doc buffer. Requires optional dependency word-extractor.
 * @param {Buffer} buffer
 * @returns {Promise<string>}
 */
async function extractDocText(buffer) {
  try {
    ...
    const parts = [];
    if (doc.getBody()) parts.push(doc.getBody().trim());
    if (doc.getHeaders && doc.getHeaders()) parts.push(doc.getHeaders().trim());
    if (doc.getFootnotes && doc.getFootnotes()) parts.push(doc.getFootnotes().trim());
    if (doc.getEndnotes && doc.getEndnotes()) parts.push(doc.getEndnotes().trim());
    const text = parts.filter(Boolean).join('\n\n').trim();
    return text;
  } catch (e) {
    if (e.code === 'MODULE_NOT_FOUND') throw new Error('Word .doc extraction requires: npm install word-extractor');
    throw e;
  }
}

/**
 * Import Word 97-2003 (.doc) into project memory.
 */
async function importDoc(projectId, buffer, filename) {
  if (!projectId || !buffer) return { ok: false, error: 'projectId and document data required' };
  const project = projectStore.getProject(projectId);
  if (!project) return { ok: false, error: 'Project not found' };
  let text;
  try {
    text = await extractDocText(Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer));
  } catch (e) {
    logger.warn('Project import DOC error:', e.message);
    return { ok: false, error: e.message };
  }
...
async function summarizeContent(content) {
  ...
  try {
    const data = await chatJson(llm, [{ role: 'user', content: prompt }]);
...
module.exports = { extractPdfText, extractDocxText, extractDocText, describeImage, importText, importPdf, importDoc, importDocx, importImage, summarizeContent, normalizeImageBase64 };
