...
function shouldAttachVisionImages(llm, config = getConfig()) {
  const visionModel = String(config?.ollama?.visionModel || '').trim();
  return llm?.provider === 'ollama' || !!visionModel;
}
...
function findLeastRecentlyUsedSessionId(sessionMap = sessions) {
  let oldestId = null;
  let oldestTime = Infinity;
  for (const [id, session] of sessionMap.entries()) {
    const lastUsedAt = Number(session?.lastUsedAt) || 0;
    if (lastUsedAt < oldestTime) {
      oldestId = id;
...
async function ensureSession(sessionId) {
  ...
  const maxSessions = Math.max(1, Number(cfg.maxSessions) || 8);
  if (sessions.size >= maxSessions) {
    const leastRecentlyUsedId = findLeastRecentlyUsedSessionId();
    if (leastRecentlyUsedId) await closeSession(leastRecentlyUsedId);
  }
  let playwright;
  try {
  ...
  try {
    ...
    if (name === 'browser_navigate') {
      ...
      try {
        ...
        const blockedNavigation = s.blockedRequests.find((request) =>
          request.isNavigation && request.frame === s.page.mainFrame()
        );
