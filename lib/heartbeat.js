...
async function runJob(job) {
  ...
  if (job.type === 'hivemind_consolidate') {
    try {
      ...
      const result = await hiveMemoryCurator.consolidateAllHiveScopes({ limit });
      const okCount = Array.isArray(result) ? result.filter((r) => r && r.snap).length : 0;
      await sendResultEmail(config, job, {
        subject: job.emailSubject || 'Hive mind consolidated',
        text: `Hive mind consolidated at ${new Date().toISOString()} across ${okCount} scope(s).\n`
      });
      await postWebhook(job.webhookUrl, { jobId: job.id, jobName: job.name, result: { ok: true, scopes: okCount }, timestamp: new Date().toISOString() });
      return { ok: true, scopes: okCount };
    } catch (e) {
  ...
  if (job.type === 'prompt' && job.prompt) {
    ...
    try {
      const data   = await chatJson(llm, [{ role: 'user', content: job.prompt }]);
...
function tick() {
  const config = getConfig();
  const legacyEnabled = !(config.automation && config.automation.legacyHeartbeatEnabled === false);
...
/** For UI: next run after last execution (or epoch), and parse errors. */
function getJobSchedulePreview(job) {
  if (!job || !job.schedule) return { ok: false, error: 'No schedule' };
  try {
    const last = getLastRunDate(job);
    const next = getNextRun(job.schedule, last);
    return {
      ...
      nextRunLocal: next.toLocaleString(),
      lastRunISO: job.lastRunAt || null
    };
  } catch (e) {
    return { ok: false, error: e.message };
  }
...
module.exports = { startHeartbeat, stopHeartbeat, runJob, getJobSchedulePreview };
