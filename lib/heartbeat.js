...
function tick() {
  ...
  // Project report (schedule + recipient in config.projectReport; one email for selected projects)
  const reportJob = projectReport.getReportJob();
  if (reportJob && isJobDue(reportJob)) {
    logger.info('[Heartbeat] running project report');
    projectReport.runReport().catch(e => {
