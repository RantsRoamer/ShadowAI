  ...
  client.on('room.message', async (roomId, event) => {
    try {
      ...
      if (verifyMatch) {
        ...
        if (out.ok) await client.sendText(roomId, `Linked to ShadowAI user "${out.username}". This bot chat will now appear in that user's chat list.`);
        else await client.sendText(roomId, out.error || 'Verification failed.');
        return;
      }
      if (trimmed === '/unlink') {
        const ok = channelLinks.unlinkChannel(username);
        await client.sendText(roomId, ok ? 'Unlinked this bot chat from web user account.' : 'This bot chat was not linked.');
        return;
      }
      if (trimmed === '/whoami') {
      ...
      if (trimmed === '/stats') {
        ...
        await client.sendText(roomId, enabled ? 'Token stats are now ON for this chat.' : 'Token stats are now OFF for this chat.');
        return;
      }
      if (['/reset', '/new'].includes(trimmed)) {
        try {
          ...
          logger.warn('Matrix reset/new error:', e.message);
          try {
      ...
      try {
        ...
        const { content: reply, tokenStats } = await chatRunner.runChatTurn({
          user: username,
        ...
        const finalReply = statsLine ? `${reply}\n\n${statsLine}` : reply;
        messages.push({ role: 'assistant', content: finalReply });
        chatStore.writeChat(username, messages);

        for (let i = 0; i < finalReply.length; i += CHUNK) {
          const part = finalReply.slice(i, i + CHUNK);
