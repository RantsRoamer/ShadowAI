...
async function startMatrixBotAsync() {
  ...
  client.on('room.encrypted_event', (_roomId, _ev) => {
    logger.warn(
      'Matrix bot: received encrypted event in room (E2E not enabled). The bot only sees plaintext. Use an unencrypted room: in Element create a room and turn encryption off, or pick a public room without E2E.'
    );
  });
  client.on('room.failed_decryption', (_roomId, _ev, err) => {
    logger.warn('Matrix bot: could not decrypt event in room — use an unencrypted room. ' + (err && err.message ? err.message : ''));
  });
    ...
    try {
      ...
      if (encrypted) {
        await client.sendText(
          roomId,
          'Hi! I joined your DM, but this room is encrypted so I cannot read messages yet. Please create a new unencrypted DM/room with me to chat.'
        );
      } else {
  ...
  client.start().then(() => {
    logger.info('Matrix bot: sync loop running — invite the bot to a room and send a message (unencrypted rooms only)');
  }).catch((e) => {
