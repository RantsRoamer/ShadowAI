...
function evaluateConditionExpression(expression, context) {
  ...
  try {
    // Use an isolated VM context instead of new Function — this prevents
    // expressions from accessing require, process, or any server-side globals.
    const sandbox = vm.createContext({ context: Object.assign({}, context) });
    return !!vm.runInContext('(' + expr + ')', sandbox, { timeout: 200 });
  } catch (_) {
    return false;
  }
}
...
async function executeNode(node, context, config, runMeta) {
  ...
  switch (node.type) {
    ...
    case 'email': {
      if (process.env.NODE_ENV === 'test' || process.env.SHADOWAI_TEST_MODE === '1') {
        observability.appendEvent({
          runId: runMeta.id,
          pipelineId: runMeta.pipelineId,
          nodeId: node.id,
          ...
          stage: 'node_end',
          status: 'success',
          message: 'Email skipped in test mode'
        });
        return 'Email skipped (test mode)';
      }
