...
function buildSystemPrompt(customInstructions = '') {
  ...
  // Timezone-aware current time context so the model answers "what time is it"
  // using the configured timezone (or server local time if none is set).
  try {
    const config = getConfig();
    const tz = typeof config.timezone === 'string' && config.timezone.trim() ? config.timezone.trim() : '';
    const now = new Date();
    let timeStr;
    if (tz) {
      try {
        ...
        timeStr = fmt.format(now) + ` (timezone: ${tz})`;
      } catch (_) {
        const fmt = new Intl.DateTimeFormat(undefined, { dateStyle: 'full', timeStyle: 'long' });
        timeStr = fmt.format(now) + ' (server local time; configured timezone invalid: ' + tz + ')';
      }
    } else {
      const fmt = new Intl.DateTimeFormat(undefined, { dateStyle: 'full', timeStyle: 'long' });
      timeStr = fmt.format(now) + ' (server local time; no explicit timezone configured)';
    }
 ...
 */
function buildProjectSystemPrompt(projectId, customInstructions = '', includeMemoryInSystem = true) {
  ...
  // Include the same time context here so project chats answer time questions correctly.
  try {
    const config = getConfig();
    const tz = typeof config.timezone === 'string' && config.timezone.trim() ? config.timezone.trim() : '';
    const now = new Date();
