...
function buildSystemPrompt(customInstructions = '', userContext = '') {
  ...
  text += '\n\n**Scheduled tasks (Heartbeat):** When the user asks for something to run on a schedule (e.g. "run X every day at 7am", "email me the weather at 8am"), you MUST (1) create_skill — create a skill that does the work (e.g. fetch data, compute something, return a string or { subject, text }). The skill runs in isolation and returns content; (2) add_heartbeat_job — add a job with type "skill", that skillId, schedule (cron e.g. 0 7 * * * for 7am server time), and if they want the result emailed set emailResult: true and emailSubject. The Heartbeat system is an internal scheduler that checks every 30 seconds and runs jobs when their cron is due. No special job types—always use a skill plus a heartbeat job. When the user asks to CHANGE an existing scheduled task or skill, use list_heartbeat_jobs to find the job id, then update_heartbeat_job (e.g. change schedule or name), or update_skill to change what the skill does. To REMOVE a scheduled job, use delete_heartbeat_job with that job id.';
  try {
    const al = getConfig().agentLoop || {};
    if (al.chatHistorySearch !== false) {
      text += '\n\n**Past chats:** You can call search_chat_history with keywords to find what was said in earlier conversations for this user (indexed transcripts). Use when they ask "what did we decide" or to recall an old detail.';
    }
    if (al.subagentMissions !== false) {
      text += '\n\n**Delegated missions:** Use dispatch_subagent_mission to queue parallel background work through Command Center (tasks show on /autoagent). Use for complex multi-step requests, not for trivial one-shot answers.';
    }
  } catch (_) {}
  } catch (e) {
...
function buildProjectSystemPrompt(projectId, customInstructions = '', includeMemoryInSystem = true, userContext = '') {
  ...
  // Apply only personality tone in project chats. Do not inject global
  // user memory/behavior facts here, because those can bleed stale or
  // cross-project data into isolated project reasoning.
  try {
    const personality = personalityLib.readPersonality(userContext).trim();
    if (personality) {
