...
function buildProjectSystemPrompt(projectId, customInstructions = '') {
  ...
  if (memory) {
    text += '--- Project memory (current; use this to answer questions—it is loaded fresh for each message so anything just added is available) ---\n' + memory + '\n--- End project memory ---\n\n';
  } else {
    text += 'The project has no memory yet. The user can add notes or import documents/images to build context.\n\n';
  }
  text += 'Be concise and helpful. Use markdown for structure. If the user asks about something not in the project memory, say so and suggest they add or import the information. When the user shares important information about this project (e.g. decisions, key dates, contacts, requirements, notes they want remembered), use the append_project_memory tool to save it to this project\'s memory so it is available for future questions.';
  const custom = (customInstructions || '').trim();
  if (custom) {
