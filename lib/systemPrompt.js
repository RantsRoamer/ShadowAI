...
function buildProjectSystemPrompt(projectId, customInstructions = '') {
  ...
  if (memory) {
    text += '--- Project memory (use this to answer questions) ---\n' + memory + '\n--- End project memory ---\n\n';
  } else {
    text += 'The project has no memory yet. The user can add notes or import documents/images to build context.\n\n';
  }
  text += 'Be concise and helpful. Use markdown for structure. If the user asks about something not in the project memory, say so and suggest they add or import the information.';
  const custom = (customInstructions || '').trim();
  if (custom) {
