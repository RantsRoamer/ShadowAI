...
async function createUser({ username, password, role }) {
  ...
  if (!/^[a-zA-Z0-9_-]{1,64}$/.test(cleanUser)) {
    throw new Error('Username must contain only letters, numbers, hyphens, and underscores (max 64 chars)');
  }
...
async function deleteUser(username) {
  ...
  const target = await new Promise((resolve, reject) => {
    db.get('SELECT role FROM users WHERE username = ?', [cleanUser], (err, row) => {
      if (err) return reject(err);
      resolve(row || null);
    });
  });
...
function authMiddleware(req, res, next) {
  ...
  if (req.path === '/login' || req.path === '/api/login' || req.path === '/api/auth' ||
      ...
      req.path.startsWith('/api/pipelines/webhook/') ||
