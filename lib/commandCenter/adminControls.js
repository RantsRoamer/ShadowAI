...
function clearAllMemory({ scopeUser = '' } = {}) {
  ...
  // 2) Remove Command Center tasks for this scope (or all CC tasks when scope is global wipe).
  let deletedTasks = 0;
  try {
    const index = agentStore.listTasks();
    for (const entry of index) {
      ...
      if (!isCommandCenterTask) continue;
      // Empty scopeUser (Master Kill / global wipe): delete all CC tasks.
      // Scoped clear: only delete CC tasks owned by that user.
      const ownerMatch = !userKey || owner === String(scopeUser || '');
      if (ownerMatch) {
