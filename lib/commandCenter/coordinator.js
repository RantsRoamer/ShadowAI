...
async function dispatchMission(text, { user = '' } = {}) {
  ...
  hiveStore.appendEvent({ type: 'mission_received', source: 'command_center', missionId, message: safeText(text, 2000), payload: { user } });

  let triage;
  try {
    triage = await triageMission(text, { user });
  } catch (e) {
    hiveStore.appendEvent({ type: 'mission_triage_failed', source: 'command_center', missionId, message: e.message });
  ...
  hiveStore.updateSnapshot({ missions: missions.slice(0, 50) });
