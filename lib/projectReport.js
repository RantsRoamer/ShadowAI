...
function sectionCanonicalKey(title) {
  ...
  if (!lower) return '';
  if (lower.startsWith('financial summary')) return 'financial summary';
  if (lower.startsWith('budget status')) return 'budget status';
  if (lower.startsWith('payment history')) return 'payment history';
  if (lower.startsWith('upcoming payments')) return 'upcoming payments';
  if (lower.startsWith('project status update')) return 'project status update';
  if (lower.startsWith('last updated')) return 'last updated';
  return lower;
}

/**
 * Consolidate memory for reporting so stale financial sections do not
 * override newer figures when the model formats budget tables.
 */
function prepareMemoryForReport(memory) {
  const text = String(memory || '').replace(/\r\n/g, '\n').trim();
  if (!text) return '';
  ...
  for (const part of parts) {
    ...
    const key = sectionCanonicalKey(title);
    if (!latestByKey.has(key)) sectionOrder.push(key);
    latestByKey.set(key, part);
  }
  const merged = [];
  if (preamble.length) merged.push(preamble.join('\n\n'));
  for (const key of sectionOrder) merged.push(latestByKey.get(key));
  return merged.join('\n\n').replace(/\n{3,}/g, '\n\n').trim();
}

function excerptLatest(memory, maxLen = MEMORY_EXCERPT_LEN) {
  const text = String(memory || '').trim();
  if (!text) return '';
  if (text.length <= maxLen) return text;
  // Keep the most recent content instead of stale opening sections.
...
function getRawProjectData(projectIds) {
  ...
  for (const id of projectIds) {
    ...
    if (!project) continue;
    const memoryRaw = projectStore.readProjectMemory(id).trim();
    const memory = prepareMemoryForReport(memoryRaw);
    const excerpt = memory
      ? excerptLatest(memory, MEMORY_EXCERPT_LEN)
...
async function formatReportWithPrompt(projectIds, reportPrompt, userContext = '') {
  ...
  if (projectIds.length === 1 && raw[0]) {
    const info = raw[0];
    const fullMemoryRaw = projectStore.readProjectMemory(projectIds[0]).trim() || info.memoryExcerpt;
    const fullMemory = prepareMemoryForReport(fullMemoryRaw);
...
module.exports = {
  ...
  runReport,
  sendReportNow,
  markReportRunStarted,
  getReportJob: getReportJobs,
  getReportJobs,
  getReports,
  newReportId,
  normalizeReport,
  persistReports,
  prepareMemoryForReport,
  excerptLatest
};
