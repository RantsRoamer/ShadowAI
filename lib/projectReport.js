...
async function formatReportWithPrompt(projectIds, reportPrompt) {
  ...
  let systemMessage = 'You format project report content for an email. The user provides instructions on what to include and how to format, then raw project data. Output only the formatted email body. No preamble, no "Here is the report", no explanation. You may use HTML (e.g. <p>, <ul>, <li>, <strong>, <h2>) or markdown; both will be rendered as HTML in the email.';
  try {
