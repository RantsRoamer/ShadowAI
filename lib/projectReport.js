...
// In-memory guard to avoid accidental duplicate sends for the same report
// within a very short window (e.g. if a job fires twice or the user clicks
// "Send now" repeatedly).
const recentSends = new Map(); // reportId -> timestamp (ms)
...
async function formatReportWithPrompt(projectIds, reportPrompt) {
  ...
  const systemMessage = 'You format project report content for an email. The user provides instructions on what to include and how to format, then raw project data. Output only the formatted email body. No preamble, no "Here is the report", no explanation. You may use HTML (e.g. <p>, <ul>, <li>, <strong>, <h2>) or markdown; both will be rendered as HTML in the email.';
  try {
...
async function runReport(report) {
  ...
  try {
    ...
    const lastMs = recentSends.get(report.id) || 0;
    if (nowMs - lastMs < 5000) {
      logger.warn('[ProjectReport] Skipping duplicate send for report', report.name || report.id, '- too soon since last send.');
      return false;
    }
    ...
    recentSends.set(report.id, nowMs);
