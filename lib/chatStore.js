...
function safeUsername(username) {
  const str = (username && typeof username === 'object') ? (username.username || '') : String(username || '');
  return str.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 64) || 'default';
}
...
const CHANNEL_PREFIXES = ['channel_', 'telegram_', 'discord_', 'matrix_', 'project_'];
function isChannelUsername(safeName) {
  return typeof safeName === 'string' && CHANNEL_PREFIXES.some(p => safeName.startsWith(p));
}

/** List all channel users (channel_*, telegram_*, discord_*, matrix_*) and their chats for the web UI. Excludes project_* (project chats live only on the project page). */
function listAllChannelChats() {
  ...
  for (const file of files) {
    ...
    if (safeName.startsWith('project_')) continue;
    try {
      ...
      else if (safeName.startsWith('matrix_')) label = 'Matrix (' + safeName.slice(7) + ')';
