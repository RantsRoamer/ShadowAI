...
async function runChatTurn(options) {
  ...
  const browserSessionId = 'channel:' + ((typeof optSessionId === 'string' && optSessionId.trim())
    ? optSessionId.trim()
    : String(user || 'channel'));
  ...
  const systemContent = systemPrompt.buildSystemPrompt(customInstructions, memoryScopeUser);
  ...
  const appendMemoryTool = {
    ...
    function: {
      name: 'append_memory',
      description: 'Save a durable user fact to memory for future chats. Use both when the user asks to remember and when a stable, future-useful detail appears naturally (identity, preferences, important dates, goals). Avoid transient details.',
  ...
  const getMemoryTool = {
    ...
    function: {
      name: 'get_memory',
      description: 'Get a structured memory value from data/memory.json by key.',
  ...
  const setMemoryTool = {
    ...
    function: {
      name: 'set_memory',
      description: 'Set a structured memory value in data/memory.json.',
      parameters: {
        type: 'object',
        required: ['key', 'value'],
      ...
      for (const tc of toolCalls) {
        ...
        try {
          ...
          if (name === 'append_memory') {
            ...
            if (text) personalityLib.appendMemory(text, memoryScopeUser);
            ...
            toolContent = key ? structuredMemory.getMemory(key, memoryScopeUser) : '';
