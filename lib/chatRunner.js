...
async function runChatTurn(options) {
  ...
  const systemContent = systemPrompt.buildSystemPrompt(customInstructions, user);
      ...
      for (const tc of toolCalls) {
        ...
        try {
          ...
          if (name === 'append_memory') {
            ...
            if (text) personalityLib.appendMemory(text, user);
            ...
            toolContent = key ? structuredMemory.getMemory(key, user) : '';
          } else if (name === 'set_memory') {
            const key = args.key != null ? String(args.key).trim() : '';
            const value = args.value != null ? String(args.value) : '';
            if (!key) toolContent = 'Error: key is required.';
            else {
              structuredMemory.setMemory(key, value, user);
