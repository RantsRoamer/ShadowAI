...
function readBehavior() {
  ensureDataDir();
  if (!fs.existsSync(BEHAVIOR_PATH)) return '';
  return fs.readFileSync(BEHAVIOR_PATH, 'utf8');
}

function writeBehavior(content) {
  ensureDataDir();
  fs.writeFileSync(BEHAVIOR_PATH, String(content ?? ''), 'utf8');
}
