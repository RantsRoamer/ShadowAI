...
async function planTask(task) {
  ...
  try {
    ...
    task.status = 'failed';
    agentStore.updateTask(task.id, { status: 'failed', log: task.log });
    return;
  }
...
async function executeStep(task) {
  ...
  if (!step) {
    agentStore.updateTask(task.id, { status: 'learning' });
    return;
  }
  ...
  while (maxRounds-- > 0) {
    ...
    try {
      ...
      agentStore.updateTask(task.id, { log: task.log });
      return;
    }
  ...
  if (stepDone) {
    ...
    agentStore.updateTask(task.id, { status: newStatus, log: task.log });
  } else {
    agentStore.updateTask(task.id, { log: task.log });
  }
}
...
module.exports = { startAgentRunner, stopAgentRunner, resumeAfterApproval, isRunnerPaused, getRunnerState };
