...
function resolveLlmForTask(task) {
  const cfg = getConfig();
  return resolveLlm(cfg, task && task.agentId ? String(task.agentId).trim() : '');
}
...
function closeBrowserSession(taskId) {
  browserTools.closeSession(agentBrowserSessionId(taskId)).catch(() => {});
}

function agentBrowserSessionId(taskId) {
  return 'agent:' + String(taskId || 'unknown');
}
...
async function executeTool(name, args, taskId) {
  ...
  if (browserTools.handles(name)) {
    return browserTools.executeBrowserTool(name, args, { sessionId: agentBrowserSessionId(taskId) });
  }
  if (name === 'web_search') {
...
function appendVisionImages(messages, images, llm) {
  if (images.length && browserTools.shouldAttachVisionImages(llm, getConfig())) {
    messages.push({
      role: 'user',
...
function appendPendingVisionImages(messages, taskId, llm) {
  appendVisionImages(messages, browserTools.takePendingVisionImages(agentBrowserSessionId(taskId)), llm);
}

function persistPendingVisionImages(task) {
  const images = browserTools.takePendingVisionImages(agentBrowserSessionId(task.id));
...
function appendTaskPendingVisionImages(messages, task, llm) {
  const images = Array.isArray(task.pendingVisionImages) ? task.pendingVisionImages : [];
  if (!images.length) return;
  if (!browserTools.shouldAttachVisionImages(llm, getConfig())) return;
  appendVisionImages(messages, images, llm);
  task.pendingVisionImages = [];
...
async function planTask(task) {
  ...
  const roleAppendix = role && role.systemAppendix ? String(role.systemAppendix).trim() : '';
  const systemMsg = [
...
async function executeStep(task) {
  ...
  appendTaskPendingVisionImages(messages, task, llm);
  ...
  while (maxRounds-- > 0) {
    ...
    let data;
    try {
      data = await chatWithTools(llm, messages, tools);
    } catch (e) {
    ...
    appendPendingVisionImages(messages, task.id, llm);
